...
class OmniActivationRealLinearMethod(QuantizeMethodBase):
    ...
    def _build_triton_w6a6_runtime(self, layer: torch.nn.Module) -> bool:
        ...
        experimental_w4a8_fastpath = (
            _env_flag("OMNI_EXPERIMENTAL_W4A8_FASTPATH")
            and weight_bits == 4
            and activation_bits == 8
        )
        if weight_bits < 6 and not experimental_w4a8_fastpath:
            return False
        ...
        if (
            experimental_w4a8_fastpath
            and not activation_symmetric
            and not activation_disable_zero_point
            and activation_bits == 8
        ):
            triton_weight_correction = (
                adjusted_weight.to(dtype=torch.float32).sum(dim=-1) * grouped_scales
            ).sum(dim=1).contiguous()
            if "omni_triton_weight_correction" in layer._buffers:
                layer._buffers["omni_triton_weight_correction"] = triton_weight_correction
            else:
                layer.register_buffer(
                    "omni_triton_weight_correction",
                    triton_weight_correction,
                    persistent=False,
                )
    ...
    def apply(
        ...
        if (
            ...
            return omni_w6a6_linear(
                ...
                activation_weight_correction=getattr(
                    layer,
                    "omni_triton_weight_correction",
                    None,
                ),
