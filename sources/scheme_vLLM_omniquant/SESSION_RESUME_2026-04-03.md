...
- The measurements below were collected on an older pre-upgrade stack.
- The current wheel-build target for this repo is `torch==2.10.0+cu129` / `vllm==0.19.0`.

## Historical environment constraints

- This section is legacy session context only.
- Do not treat it as the current build target.
...
```text
작업 디렉터리는 /home/ubuntu/vLLM_OmniQuant 이고, 현재 기준 버전은 torch==2.10.0+cu129 / vllm==0.19.0 이다. SESSION_RESUME_2026-04-03.md 를 먼저 읽되, 측정값은 legacy 기록으로 취급해라. 현재 w6a6 1ep rebuilt 체크포인트 관련 맥락을 참고해 vllm/vllm/model_executor/layers/quantization/omni_activation_real.py 를 검토하고, 필요한 wheel build / smoke / eval 절차를 현재 버전 기준으로 다시 맞춰라.
```
