...
def _resolve_local_or_downloaded_model_dir(
    model_config: ModelConfig,
    load_config: LoadConfig,
    *,
    allow_patterns: str | list[str] = "*.json",
) -> str:
    model_name_or_path = (
...
def _load_quant_config_file_from_model_dir(
    hf_folder: str,
    possible_config_filenames: list[str],
    ...
    if len(quant_config_files) > 1:
        raise ValueError(
            "Found multiple quantization config files: "
            f"{quant_config_files}"
        )

    quant_config_file = quant_config_files[0]
    with open(quant_config_file) as f:
        return json.load(f), quant_config_file
...
def get_quant_config(
    ...
    file_quant_config = None
    if possible_config_filenames and (
        hf_quant_config is None or hasattr(quant_cls, "merge_with_file_config")
    ):
        hf_folder = _resolve_local_or_downloaded_model_dir(model_config, load_config)
        file_quant_config, _ = _load_quant_config_file_from_model_dir(
            hf_folder,
            possible_config_filenames,
        )

    if hf_quant_config is not None:
        if file_quant_config is not None and hasattr(quant_cls, "merge_with_file_config"):
            hf_quant_config = quant_cls.merge_with_file_config(
                hf_quant_config,
                file_quant_config,
            )
    ...
    if not possible_config_filenames:
        return quant_cls()
    if file_quant_config is None:
        file_quant_config, quant_config_file = _load_quant_config_file_from_model_dir(
            hf_folder,
            possible_config_filenames,
        )
    else:
