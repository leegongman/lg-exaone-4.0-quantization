...
class ModelConfig:
    ...
    def _verify_quantization(self) -> None:
        ...
        if quant_cfg is not None:
            ...
            if (
                quant_method in supported_quantization
                and quant_method not in get_args(me_quant.QuantizationMethods)
            ):
                # Custom quantization methods are already explicit in the
                # checkpoint config, so we can trust them directly instead of
                # probing every built-in override backend and importing their
                # optional dependencies.
                if self.quantization is None:
                    self.quantization = quant_method
                elif self.quantization != quant_method:
                    raise ValueError(
                        "Quantization method specified by the checkpoint "
                        f"({quant_method}) does not match the user-requested "
                        f"method ({self.quantization})."
                    )
            else:
