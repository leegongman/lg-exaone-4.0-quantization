...
if current_platform.is_cuda():
    ...
    try:
        ...
        logger.warning_once(
            "Bundled flash-attn extension is unavailable on CUDA; "
            "falling back to non-flash attention backends: %s",
            exc,
        )

        def flash_attn_varlen_func(*args: Any, **kwargs: Any) -> Any:  # type: ignore[no-redef,misc]
            raise ImportError(
                "CUDA flash-attn extension is unavailable. "
                "Select TRITON_ATTN or another supported attention backend."
            )
