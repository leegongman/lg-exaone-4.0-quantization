...
class OmniActivationRealConfig(QuantizationConfig):
    ...
    def _get_effective_layer_config(self, prefix: str) -> dict[str, Any]:
        ...
        if proj_name in self.packed_modules_mapping:
            ...
            first = non_empty_configs[0]
            for shard_config in non_empty_configs[1:]:
                if not _json_deep_equal(first, shard_config):
                    raise ValueError(
                        f"Packed EXAONE layer {prefix} received mismatched omni "
                        "activation real shard configs. q/k/v and gate/up "
                        "must agree before they can be attached to a fused "
                        "vLLM linear module."
                    )
            return dict(first)
    ...
    def get_quant_method(
        ...
        if isinstance(layer, Attention) and self._attention_enabled(prefix):
...
class OmniActivationRealLinearMethod(QuantizeMethodBase):
    ...
    def _resolve_shard_input_meta(
        ...
        if input_meta.ndim == 1 and input_meta.numel() > 1:
            if input_meta.numel() == x.shape[-1]:
                return _expand_input_metadata(input_meta, x, granularity, group_size)
            if input_meta.numel() > shard_idx:
                return input_meta[shard_idx].reshape(1, 1)
    ...
    def _build_runtime(
        ...
        for shard_idx, shard_size in enumerate(output_partition_sizes):
            ...
            shard_runtimes.append(
                _OmniShardRuntime(
                    ...
                    input_scale=(
                        input_scale[shard_idx].reshape(1, 1)
                        if input_scale is not None
                        and input_scale.ndim == 1
                        and input_scale.numel() == len(output_partition_sizes)
                        else input_scale
                    ),
                    input_zero_point=(
                        input_zero_point[shard_idx].reshape(1, 1)
                        if input_zero_point is not None
                        and input_zero_point.ndim == 1
                        and input_zero_point.numel() == len(output_partition_sizes)
        ...
        layer.omni_input_scale_granularity = input_scale_granularity
        layer.omni_input_group_size = input_group_size
        return dense_weight, layer.omni_shard_runtimes

    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        self._build_runtime(layer)
    ...
    def _fake_quantize_input(
        ...
        scale = _expand_input_metadata(
            input_scale.to(dtype=x.dtype),
            x,
            cast(str, getattr(layer, "omni_input_scale_granularity")),
            cast(int | None, getattr(layer, "omni_input_group_size")),
        )

        zero_point = (
            _expand_input_metadata(
                input_zero_point.to(dtype=x.dtype),
                x,
                cast(str, getattr(layer, "omni_input_scale_granularity")),
                cast(int | None, getattr(layer, "omni_input_group_size")),
            )
    ...
    def apply(
        ...
        _, shard_runtimes = self._build_runtime(layer)
...
def maybe_run_omni_attention(
    ...
    # Import lazily to avoid pulling vllm.config/forward_context into the
    # quantization package initialization path.
